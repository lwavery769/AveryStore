...
namespace ALStore {
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		if (attachmentIndex > m_ColorAttachments.size()) {
			std::cout << "attachmentIndex > m_ColorAttachments.size" << std::endl;
		};

		auto& spec = m_ColorAttachmentSpecifications[attachmentIndex];
		GLuint clearColor[4] = { 10, 20, 1, 0 };
		//glClearTexImage(texture, 0, GL_BGRA, GL_UNSIGNED_BYTE, &clearColor);
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::HazelFBTextureFormatToGL(spec.TextureFormat), GL_RED_INTEGER, &value);
	}
	int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		return p;
		//return glm::vec3(pixel_data[0], pixel_data[1], pixel_data[2]);
